...
def EB(df):
    mask = ((abs(df.etaSCEle1) > EBthreshold) & (abs(df.etaSCEle2) < EBthreshold)) | (
        (abs(df.etaSCEle1) < EBthreshold) & (abs(df.etaSCEle2) > EBthreshold)
    )
    return df[mask]
...
def BBl(df):
    ...
    mask = (
        (abs(df.etaSCEle1) < EBthreshold)
        & (abs(df.etaSCEle2) < EBthreshold)
        & (df.timeSeedSC1_recal != 0.0)
        & (df.timeSeedSC2_recal != 0.0)
    )
    return df[mask]
...
def relAmp(df):
    mask = (
        abs(df.amplitudeSeedSC1 - df.amplitudeSecondToSeedSC1)
        / (df.amplitudeSeedSC1 + df.amplitudeSecondToSeedSC1)
        < 0.1
    )
    return df[mask]
...
def relAmp2(df):
    mask = (
        abs(df.amplitudeSeedSC2 - df.amplitudeSecondToSeedSC2)
        / (df.amplitudeSeedSC2 + df.amplitudeSecondToSeedSC2)
        < 0.1
    )
    return df[mask]
...
def EtaGt0p4Lt0p8_1(df):
    mask = (abs(df.etaSCEle1) < 0.8) & (abs(df.etaSCEle1) > 0.4)
    return df[mask]


def EtaGt0p8Lt1p4_1(df):
    mask = (abs(df.etaSCEle1) < 1.4) & (abs(df.etaSCEle1) > 0.8)
    return df[mask]

def EtaLt0p4_2(df):
    mask = abs(df.etaSCEle2) < 0.4
    return df[mask]


def EtaGt0p4Lt0p8_2(df):
    mask = (abs(df.etaSCEle2) < 0.8) & (abs(df.etaSCEle2) > 0.4)
    return df[mask]


def EtaGt0p8Lt1p4_2(df):
    mask = (abs(df.etaSCEle2) < 1.4) & (abs(df.etaSCEle2) > 0.8)
    return df[mask]
...
def Run1Sel_e1(df):
    mask = (
        (
            df.amplitudeSeedSC1
            * np.where(abs(df.etaSCEle1) > EBthreshold, ADC2GEV_E, ADC2GEV_B)
        )
        < 120
    ) & (
        (
            df.amplitudeSecondToSeedSC1
            * np.where(abs(df.etaSCEle1) > EBthreshold, ADC2GEV_E, ADC2GEV_B)
        )
        < 120
    ) & abs(
        df.amplitudeSeedSC1.div(df.amplitudeSecondToSeedSC1)
    ) < 1.2
...
def Run1Sel_e2(df):
    mask = (
        (
            df.amplitudeSeedSC2
            * np.where(abs(df.etaSCEle2) > EBthreshold, ADC2GEV_E, ADC2GEV_B)
        )
        < 120
    ) & (
        (
            df.amplitudeSecondToSeedSC2
            * np.where(abs(df.etaSCEle2) > EBthreshold, ADC2GEV_E, ADC2GEV_B)
        )
        < 120
    ) & abs(
        df.amplitudeSeedSC2.div(df.amplitudeSecondToSeedSC2)
    ) < 1.2
...
def sameRO(df):
    mask = (df.iTTSeedSC1 == df.iTTSecondToSeedSC1) & (
        df.scSeedSC1 == df.scSecondToSeedSC1
    )
    return df[mask]
...
def Amp300to600(df):
    mask = (
        (df.amplitudeSeedSC1 > 300)
        & (df.amplitudeSeedSC2 > 300)
        & (df.amplitudeSeedSC1 < 600)
        & (df.amplitudeSeedSC2 < 600)
    )
    return df[mask]
...
def Amp600to900(df):
    mask = (
        (df.amplitudeSeedSC1 > 600)
        & (df.amplitudeSeedSC2 > 600)
        & (df.amplitudeSeedSC1 < 900)
        & (df.amplitudeSeedSC2 < 900)
    )
    return df[mask]
...
def Amp900to1200(df):
    mask = (
        (df.amplitudeSeedSC1 > 900)
        & (df.amplitudeSeedSC2 > 900)
        & (df.amplitudeSeedSC1 < 1200)
        & (df.amplitudeSeedSC2 < 1200)
    )
    return df[mask]
...
def Amp1200to1500(df):
    mask = (
        (df.amplitudeSeedSC1 > 1200)
        & (df.amplitudeSeedSC2 > 1200)
        & (df.amplitudeSeedSC1 < 1500)
        & (df.amplitudeSeedSC2 < 1500)
    )
    return df[mask]
...
def Amp1500to1800(df):
    mask = (
        (df.amplitudeSeedSC1 > 1500)
        & (df.amplitudeSeedSC2 > 1500)
        & (df.amplitudeSeedSC1 < 1800)
        & (df.amplitudeSeedSC2 < 1800)
    )
    return df[mask]
...
def Amp1800to2500(df):
    mask = (
        (df.amplitudeSeedSC1 > 1800)
        & (df.amplitudeSeedSC2 > 1800)
        & (df.amplitudeSeedSC1 < 2500)
        & (df.amplitudeSeedSC2 < 2500)
    )
    return df[mask]
