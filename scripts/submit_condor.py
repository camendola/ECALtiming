...
parser.add_argument('-f', '--file', 
                    default=2017, help="file with commands to run")
...
variables = [line.rstrip('\n') for line in open(file) if line.split("=")[0].isupper() and not line.startswith("#")]
linestolaunch = [line.rstrip('\n') for line in open(file) if line.startswith(args.pattern)]


print(linestolaunch, variables)
...
for line in linestolaunch:
    ...
    job_file.write ('source /afs/cern.ch/user/c/camendol/ECALtiming/setup.sh\n')
    job_file.write ('cd /afs/cern.ch/user/c/camendol/ECALtiming;\n')
    ...
    submit_file.write ('Log         = condor_$(ProcId).log\n')
    submit_file.write ('Output      = condor_$(ProcId).out\n')
    submit_file.write ('Error       = condor_$(ProcId).error\n')
    submit_file.write ('queue 1\n')
    submit_file.write ('+JobFlavour = "microcentury"\n')
    submit_file.close ()
